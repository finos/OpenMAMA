...
def get_command_line_opts( host, products, VERSIONS ):
    ...
    if host['os'] == 'Windows':
        env = Environment()
        opts.AddVariables(
            ListVariable('buildtype', 'Windows Build type e.g dynamic', 'all',
                         names=['dynamic','dynamic-debug','static','static-debug'] ),
            ...
            PathVariable('nunit_home', 'Path to Nunit home', None,
                         PathVariable.PathIsDir),
            PathVariable('libevent_home', 'Path to libevent Libraries', None,
                         PathVariable.PathAccept),
            EnumVariable('target_arch', 'Specifies if the build should target 32 or 64 bit architectures.', host['arch'],
    ...
    if host['os'] == 'Linux':
        opts.AddVariables(
            ...
            PathVariable('apr_home', 'Path to Apache APR Libraries',
                '/usr/local/apr', PathVariable.PathIsDir),
        )
